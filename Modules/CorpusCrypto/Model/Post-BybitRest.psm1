...
function Post-BybitRest {
  ...
  #>
  try {
    $param_str = $serverTime + $Cred.bybitAPI + "5000" + $body
  ...
  try {
    $response = "error"
    $response = Invoke-RestMethod -Uri ($Config.bybitURL + $endpoint) -Method Post -Headers $header -Body $body
